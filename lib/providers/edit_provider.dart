...
///
/// 核心功能：
/// - params：当前编辑参数（实时更新，驱动 GPU shader 预览）
/// - isDirty：参数与数据库不同步时为 true
/// - undoStack/redoStack：内存级撤销/重做，零延迟
/// - snapshots：持久化快照（命名快照，跨会话可用，数据库中）
/// - history：持久化操作历史（可回退到任意历史节点）
/// - localHistory：内存级历史条目（编辑后立即创建，实时刷新历史面板，
///   持久化到 DB 后从该列表移除）
///
/// ⚡ 性能设计：
/// - 编辑参数实时更新 → GPU shader 实时渲染预览，不等待 DB。
/// - 撤销/重做栈在内存中维护，无需查询 DB。
/// - 自动保存防抖 500ms（editAutoSaveDebounceMs），避免滑块拖动时频繁写入。
/// - 历史记录防抖 1000ms（editHistoryDebounceMs），批量合并后再持久化。
/// - 滑块拖动时 300ms coalesce 本地历史，离散操作（翻转/重置）立即创建。
